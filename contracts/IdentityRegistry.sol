...
contract IdentityRegistry {
    ...
    function hasClaim(address _subject, bytes32 _key) external view returns (bool) {
        ...
        if (address(claims) == 0) return 0;
        return claims.hasClaim(0, _key);
    }
    ...
    function verifyClaim(address _issuer, address _subject, bytes32 _key) external pure returns (ClaimRegistry.Claim) {
        ClaimRegistry.Claim memory claim = this.getClaim(_issuer, _subject, _key);
        require(address(claim) != 0 && checkClaimStruct(claim, _subject));
        return claim;
    }

    function verifyClaim(address[] _issuers, address _subject, bytes32 _key) external pure returns (ClaimRegistry.Claim) {
        ClaimRegistry.Claim memory claim = this.getClaim(_issuers, _subject, _key);
        require(address(claim) != 0 && checkClaimStruct(claim, _subject));
        return claim;
    }

    function checkClaimStruct(ClaimRegistry.Claim _claim, address _subject) internal pure returns (bool) {
        bytes32 prefixedHash = keccak256("\x19Ethereum Signed Message:\n32", _claim.issuer, _claim.key, _claim.value, _claim.nonce, _claim.expires);
        return ecrecover(prefixedHash, _claim.sigV, _claim.sigR, _claim.sigS) == _subject ? _claim : ClaimRegistry.Claim(0);
    }
