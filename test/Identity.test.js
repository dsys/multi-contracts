...
contract("Identity", accounts => {
  ...
  describe("executing signed messages", () => {
    ...
    it.only("executes signed messages from management keys", async () => {
      const args = [
        identity.address, // to
      ...
      const messageSignature = web3.eth.sign(accounts[0], messageHash);
    ...
    it.only("does not execute signed messages with invalid signatures", async () => {
      const args = [
        identity.address, // to
        identity.address, // from
        0, // value
    ...
    it.only("does not execute signed messages from action keys that operate on the contract itself", async () => {
